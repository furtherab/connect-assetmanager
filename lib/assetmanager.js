...
module.exports = function assetManager (settings) {
		...
		settings.forEach(function (group, groupName) {
			if (!group.stale) {
				group.files.forEach(function (file, index) {
					...
					fs.watchFile(path.join(group.path, file), function (old, newFile) {
	...
	this.generateCache = function (generateGroup) {
		...
		settings.forEach(function (group, groupName) {
			...
			userAgentMatches.forEach(function(match) {
				var basePath = group.path;
				Step(function () {
					var grouping = this.group();
					group.files.forEach(function (file) {
						if (!generateGroup || generateGroup && groupName === generateGroup) {
							self.getFile(file, basePath, groupName, grouping());
						}
					});
				}, function (err, contents) {
					if (err) {
	...
	this.manipulate = function (manipulateInstructions, fileContent, basePath, index, last, callback) {
		if (manipulateInstructions && Array.isArray(manipulateInstructions)) {
				var callIndex = 0;
				(function modify(content, basePath, index, last) {
					if (callIndex < manipulateInstructions.length) {
						callIndex++;
						manipulateInstructions[callIndex-1](content, basePath, index, last, function (content) {
							modify(content, basePath, index, last);
						});
					} else {
						callback(null, content);
					}
				})(fileContent, basePath, index, last);
		} else if (manipulateInstructions && typeof manipulateInstructions === 'function') {
			manipulateInstructions(fileContent, basePath, index, last, callback);
		} else {
			callback(null, fileContent);
		}
	};

	this.getFile = function (file, basePath, groupName, callback) {
		var isExternal = typeof(file) === 'string' && file.match(/^https?:\/\//);

		var fileInfo = {
			'filePath': isExternal ? file: path.join(basePath, file)
		};
